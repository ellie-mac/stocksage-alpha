#!/usr/bin/env python3
"""
xhs/xhs_writer.py — 小红书文案生成器

用法（从仓库根目录运行）:
    python xhs/xhs_writer.py preauction                                  # 09:25 竞价文案
    python xhs/xhs_writer.py morning  [--query "低估值高成长"] [--top 5] [--style 1|2|3|all|auto]
    python xhs/xhs_writer.py midday   [--style ...]
    python xhs/xhs_writer.py evening  [--style ...] [--no-tomorrow]
    python xhs/xhs_writer.py milestone [--force]   # Day 7/14/30 周期性复盘
    python xhs/xhs_writer.py status
    python xhs/xhs_writer.py history
"""
...
def cmd_morning(args):
    ...
    if day in MILESTONE_DAYS:
        print(f"[!] 今天是 Day {day}，里程碑节点！建议运行: python xhs/xhs_writer.py milestone\n")
...
def cmd_status(args):
    ...
    if "evening" in record:
        ...
        print("\n🌙 晚盘记录暂无（运行 python xhs/xhs_writer.py evening）")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  python xhs/xhs_writer.py morning --query "低估值高成长" --top 5
  python xhs/xhs_writer.py morning --style auto     # 根据当前 streak 自动选风格
  python xhs/xhs_writer.py evening --style 1
  python xhs/xhs_writer.py evening --no-tomorrow    # 跳过明日筛选（省2分钟）
  python xhs/xhs_writer.py milestone                # Day 7/14/30 复盘帖
  python xhs/xhs_writer.py milestone --force        # 任意时间强制生成复盘
  python xhs/xhs_writer.py history
""",
    )
