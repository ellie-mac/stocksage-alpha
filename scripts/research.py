...
def research(raw_input: str, weights: FactorWeights = DEFAULT_WEIGHTS) -> dict:
    ...
    pos_factor      = score_52w_position(price_df)

    # Extended A: spot / price-based
    div_factor   = score_dividend_yield(quote.get("div_yield", 0), financial_df)
    vr_factor    = score_volume_ratio(quote.get("volume_ratio", 0), quote.get("change_pct", 0))
    ma_factor    = score_ma_alignment(price_df, revision_df)
    lv_factor    = score_low_volatility(price_df, regime_score)
    rev_factor   = score_reversal(price_df, financial_df, revision_df)
    acc_factor   = score_accruals(financial_df)
    ag_factor    = score_asset_growth(financial_df)
    pf_factor    = score_piotroski(financial_df, price_df, _pe_pct, _pb_pct)
    si_factor    = score_short_interest(margin_df, circ_cap, price_df, revision_df)
    rsi_factor   = score_rsi_signal(price_df)
    macd_factor  = score_macd_signal(price_df)
    tpct_factor  = score_turnover_percentile(price_df)
